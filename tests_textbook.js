...
setTimeout(()=>{
  ...
  t('十三类全配完：A–K 各≥8 张，L/M 至少各一张',()=>{const by={};allImgs.forEach(x=>by[x[0]]=(by[x[0]]||0)+1);
    const miss=['A','B','C','D','E','F','G','H','I','J','K'].filter(c=>!(by[c]>=8));
  ...
  t('成套图表没有缺张',()=>{
    ...
    /* ⚠️ 2026-08-18 全库逐张看图核对：C19 原本 23 张、少的正是第 24 条「破头」——
       它被 near 拉到了 D16（砂法总表也引中级 p57）。已移回并加小节锚点，
       C19 二十四条到齐 24 张、D16 相应减到 7 张。*/
    const want=[['C','C11',9,'穴上九星'],['C','C19',24,'二十四凶穴'],
                ['B','B12',11,'九星与贪狼十二形'],['B','B19',7,'破军与他星合名'],
                ['G','G3',3,'先后天八卦与洛书九宫'],['K','K2',13,'入户门与门尺'],
                ['H','H6',4,'八煞黄泉的六爻装卦'],['I','I3',8,'玄空：六大派到飞星盘'],
                ['J','J10',9,'现场实操：在哪里下罗盘'],
